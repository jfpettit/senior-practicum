...
class monte_carlo_policy_iteration:
    ...
    def state_action_MC_control(self, epochs, epsilon):
        ...
        for ep in range(epochs):
            ...
            if ep % 100000 == 0:
                print('Time elapsed to ', ep, 'iterations: ', time.time() - start)
                Q_win_rate.append(self.record_win_rate_q_func(Q_, 10000))
                pi_win_rate.append(self.record_win_rate_policy(pi, 10000))
                np.save('Q_win_rate.npy', np.asarray(Q_win_rate))
                np.save('pi_win_rate.npy', np.asarray(pi_win_rate))
            if ep in [0, 10, 100, 1000, 10000, 100000, 1000000, epochs-1]:
                Q_wr_10.append(self.record_win_rate_q_func(Q_, 10000))
                pi_wr_10.append(self.record_win_rate_policy(pi, 10000))
                np.save('Q_wr_10.npy', np.asarray(Q_wr_10))
                np.save('pi_wr_10.npy', np.asarray(pi_wr_10))
        return Q_, Q_win_rate, Q_wr_10, pi, pi_win_rate, pi_wr_10
    
    def record_win_rate_policy(self, policy, num_games):
        player_wins = 0
        for game in range(num_games):
            s_ = self.blckjck._reset()
            for i in range(100):
                pi = policy(s_)
                action_choice = np.argmax(pi)
                new_state, reward, over, _ = self.blckjck._step(action_choice)
                if over:
                    break
                s_ = new_state
